...
func init() {
	...
	d.criteria = append(d.criteria, &HD8Check{})
	d.criteria = append(d.criteria, &HD9Check{})
	d.criteria = append(d.criteria, &HD10Check{})
	d.criteria = append(d.criteria, &HD11Check{})
	d.criteria = append(d.criteria, &HD12Check{})
	d.criteria = append(d.criteria, &HD13Check{})
	d.criteria = append(d.criteria, &HD14Check{})
	d.criteria = append(d.criteria, &HD15Check{})
	d.criteria = append(d.criteria, &HD16Check{})
	// Load YAML-defined criteria (simple flag/output checks)
	if err := d.loadCriteria(); err != nil {
		panic(fmt.Sprintf("failed to load help domain criteria: %v", err))
	}
	domains.Register("help", d)
}

// HelpDomain implements the Help & Documentation conformance domain.
type HelpDomain struct {
	criteria []domains.Criterion
}

func (d *HelpDomain) Name() string {
	return "help"
}

func (d *HelpDomain) Criteria() []domains.Criterion {
	return d.criteria
}

// loadCriteria loads YAML criteria from the embedded filesystem
func (d *HelpDomain) loadCriteria() error {
...
// yamlCriterionFile matches the YAML file structure
