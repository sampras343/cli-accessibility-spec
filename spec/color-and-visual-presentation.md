...
| **Rationale** | OSC 8 terminal hyperlinks (introduced by VTE/GNOME Terminal and iTerm2 in 2017) allow clickable URLs in terminal output — similar to HTML `<a>` tags. Tools like `ls --hyperlink`, GCC (linking error codes to documentation), and Rust's `cargo` use them. However, screen readers interact with the terminal's character buffer and do not interpret OSC sequences — the hyperlink is invisible to assistive technology. Many terminal emulators still don't support OSC 8, and unsupported terminals silently ignore the sequences (showing only the visible text). If the URL is meaningful information that users need (e.g., a documentation link), it must be discoverable without relying on the hyperlink. Additionally, OSC sequences are not SGR codes — they can survive `NO_COLOR` suppression if tools only strip SGR, leaving raw `\x1b]8;;...` bytes visible in `TERM=dumb` environments like Emacs shell buffers. There is currently no mechanism for tools to detect whether the terminal supports OSC 8, making graceful degradation essential rather than optional. |
| **Test method** | (1) Run the tool with default settings and scan for OSC 8 sequences (`\x1b]8;`). (2) If present, run with `TERM=dumb` and verify OSC 8 sequences are suppressed — terminals in dumb mode cannot interpret OSC, so raw bytes would appear as garbage. (3) Run with `NO_COLOR=1` and verify OSC 8 sequences are suppressed. (4) Verify the linked text is descriptive — not just "click here", "link", or an opaque code. The information conveyed by the hyperlink (the URL, the documentation target) must be accessible without clicking it. (5) Verify that the tool's behavior is identical (no information lost) when run in a terminal that does not support OSC 8 — the specification guarantees silent degradation, but tools must not build workflows that depend on clicking. |
| **Positive examples** | GCC links error codes like `[-Werror=implicit-function-declaration]` to documentation — the error code text is readable and searchable without the hyperlink. `ls --hyperlink` links filenames but the filenames are always visible as plain text. `cargo` links crate names in dependency output where the crate name itself is informative. |
| **Negative examples** | A tool that prints "See details" as a hyperlink with no visible URL and no other way to reach the documentation. A tool that emits OSC 8 sequences with `TERM=dumb` set, producing raw `\x1b]8;;https://...` garbage in Emacs compile buffers. A tool that only provides error documentation via hyperlink, with no `--help` or man page equivalent. |
| **Disability impact** | Visual |
...
| **Rationale** | CV-9 checks whether fixed foreground colors contrast against the *terminal's* background — but a tool can also set its OWN background color, creating a self-contained color pair that the terminal theme cannot fix. Real-world example: GNU `ls` uses `LS_COLORS` entries like `34;42` (blue text on green background, ~1.4:1 contrast ratio) for other-writable directories, `37;41` (white on red, ~5.7:1) for setuid files, and `30;43` (black on yellow, ~15.4:1) for setgid files. The blue-on-green pair is nearly unreadable for many users, particularly those with deuteranopia (green-blind) who cannot distinguish the two colors at all. Unlike CV-9's concern (foreground vs. unknown terminal background), CV-14 addresses cases where the tool controls both colors and the contrast can be evaluated directly against the WCAG formula. For 4-bit ANSI colors, the tool doesn't control the actual RGB values (the terminal theme does), but the xterm defaults provide a reasonable baseline for evaluation since they are the most common starting point. |
| **Test method** | (1) Capture output and extract all SGR sequences. (2) Parse compound SGR sequences to identify cases where both foreground (30-37, 90-97, 38;5;N, 38;2;R;G;B) and background (40-47, 100-107, 48;5;N, 48;2;R;G;B) are set within the same or adjacent sequences (before a reset). (3) Map 4-bit ANSI codes to xterm default RGB values: 0=#000000, 1=#800000, 2=#008000, 3=#808000, 4=#000080, 5=#800080, 6=#008080, 7=#C0C0C0, bright 0=#808080, bright 1=#FF0000, bright 2=#00FF00, bright 3=#FFFF00, bright 4=#0000FF, bright 5=#FF00FF, bright 6=#00FFFF, bright 7=#FFFFFF. (4) Calculate contrast ratio using the WCAG relative luminance formula: `contrast = (L1 + 0.05) / (L2 + 0.05)`. (5) Flag any pair below 4.5:1. Note: since 4-bit ANSI colors are user-customizable, this check uses xterm defaults as a baseline — actual user themes may produce different contrast ratios. Flag as SEMI for human review. |
| **Positive examples** | `30;43` (black on yellow) = ~15.4:1 contrast — excellent. `37;41` (white on red) = ~5.7:1 — passes AA. `30;47` (black on white) = 21:1 — maximum contrast. Status badges that use white text on dark backgrounds consistently pass. |
| **Negative examples** | `34;42` (blue on green) = ~1.4:1 — severely fails the 4.5:1 minimum. `31;42` (red on green) = ~1.3:1 — near-identical for red-green color blind users AND fails contrast. `36;44` (cyan on blue) = ~2.1:1 — fails. Any pastel-on-pastel combination using 8-bit or 24-bit colors that produces low luminance difference. |
| **Disability impact** | Visual |
| **WCAG mapping** | 1.4.3 Contrast (Minimum) |
| **References** | [Understanding SC 1.4.3 Contrast (Minimum)](https://www.w3.org/WAI/WCAG21/Understanding/contrast-minimum.html), [Coloring for Colorblindness (David Nichols)](https://davidmathlogic.com/colorblind/) |
...
| **Rationale** | Modern CLI tools increasingly use Unicode symbols for visual polish: checkmarks for pass/fail (vitest, pytest), arrows for navigation (fzf, pnpm), circles for status (systemctl), and emoji for categories (🚀 for deployment, 📦 for packages). While visually appealing, these symbols create three accessibility problems: (1) **Screen reader noise:** Screen readers speak the Unicode name, not the intended meaning — "BLACK CIRCLE" doesn't communicate "in progress" or "selected." A user hearing "HEAVY CHECK MARK task one HEAVY MULTIPLICATION X task two BLACK RIGHT-POINTING TRIANGLE task three" must mentally map opaque symbol names to pass/fail/running semantics. When multiple symbols appear in rapid succession (test output, status dashboards), the cognitive overhead becomes prohibitive. (2) **Tofu rendering:** Terminals without fonts covering the required Unicode ranges render symbols as empty boxes (□), called "tofu." This is common on minimal Linux installations, SSH sessions to servers, and older terminals. All visual meaning is lost — a column of tofu boxes conveys nothing. (3) **Cross-platform inconsistency:** The same emoji may render differently across terminal emulators and operating systems, sometimes as a wide character (taking 2 columns) and sometimes as narrow (1 column), breaking table alignment and output formatting. Text alternatives avoid all three problems: `[PASS]`, `[FAIL]`, `[WARN]`, `[INFO]` are universally understood, work in all terminals, render at predictable widths, and are announced literally by screen readers. |
| **Test method** | (1) Scan output for common Unicode status symbols: checkmarks (✓ U+2713, ✗ U+2717, ✔ U+2714, ✘ U+2718, ☑ U+2611, ☒ U+2612, ✅ U+2705, ❌ U+274C), circles (● U+25CF, ○ U+25CB, ◉ U+25C9, ◆ U+25C6, ◇ U+25C7, ⬤ U+2B24), arrows (▶ U+25B6, ▷ U+25B7, ► U+25BA, → U+2192, ← U+2190, ↑ U+2191, ↓ U+2193), warning/info signs (⚠ U+26A0, ⚡ U+26A1, ℹ U+2139, ⓘ U+24D8), and emoji ranges (U+1F300-U+1F9FF, U+2600-U+26FF, U+2700-U+27BF). (2) For each symbol found, check if adjacent text (within 5 characters before or after) provides equivalent semantic meaning — e.g., `✓ PASS` or `[OK] ✓` or `PASS ✓` all qualify; bare `✓` with only whitespace around it does not. (3) Flag bare symbols without text context as potential violations. (4) Check for `--plain`, `--ascii`, or `--no-unicode` flag support. The presence of such a flag is a strong mitigating factor even if bare symbols appear in default output. |
| **Positive examples** | `pytest` output: `PASSED` text with optional ✓ — the text carries the meaning. `gh auth status`: `✓ Logged in to github.com` — the ✓ supplements the text. `eslint`: `✓ file.js` alongside a text summary "N problems found." This CLI-ACS suite: `[PASS] CV-2 NO_COLOR Support` — text prefix carries full meaning, no symbol dependency. |
| **Negative examples** | A test runner that shows only `✓` or `✗` per test with no "PASS"/"FAIL" text. A status dashboard that uses `🔴`/`🟢`/`🟡` as the only indicator of service health. An interactive menu that uses `●`/`○` for selected/unselected with no text equivalent (screen reader hears "BLACK CIRCLE" / "WHITE CIRCLE" — meaningless). A deployment tool using `🚀` as the only indicator that deployment started. |
| **Disability impact** | Visual (screen readers, tofu rendering), Cognitive (opaque symbol names, cross-platform inconsistency) |
| **WCAG mapping** | 1.1.1 Non-text Content |
| **References** | [Scope: How special characters affect screen readers](https://business.scope.org.uk/accessibility-screen-readers-special-characters-and-unicode-symbols/), [Pope Tech: Making emojis and icons screen reader accessible](https://blog.pope.tech/2026/04/01/making-emojis-and-icons-screen-reader-accessible/), [Claude Code Issue #247: Screen Reader Accessibility for Unicode Symbols](https://github.com/anthropics/claude-code/issues/247) |
